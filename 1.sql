-- 1️⃣ NOT NULL
-- Create a table Students1 where student_name and phone_number must not accept NULL values.
Create table Students1(id int  auto_increment primary key , student_name varchar(20) not null, phone_number varchar(15) not null);

-- 2️⃣ NOT NULL
-- Create a table Employees1 where emp_name and department cannot be NULL.
Create table Employees1(emp_id int auto_increment  primary key, emp_name varchar(20) not null, department varchar(20) not null);
...
Create table products(product_id int auto_increment primary key, product_name varchar(10));

-- 8️⃣ AUTO_INCREMENT
-- Create a table Students2 where roll_no starts automatically from 1.
Create table students2(roll_no int auto_increment primary key, s_name varchar(10));

-- 9️⃣ CHECK
-- Create a table Employees2 where salary must be greater than 10000.
Create table Employees2(emp_id int primary key, salary int check (salary>10000));
...
-- Create a table Products2 where:
-- •	price cannot be NULL
-- •	price must be greater than 0
create table products2(price int not null check (price>0));
