...
select c.*, s.s_name from courses c right join students s on c.std_id=s.std_id;
...
select c.*, s.* from courses c right join students s on c.std_id=s.std_id;

-- 9.	Find courses where students joined after 2024-01-01 using RIGHT JOIN.
select c.c_name, s.s_name, s.join_dae from students s right join courses c on c.std_id=s.std_id where s.join_date>'2024-01-01';
